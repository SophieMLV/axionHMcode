...
def func_z_formation(M, k, PS, cosmo_dic, Omega_0, f = 0.01):
    ...
    def func_find_root(x, Mass):
        nu = func_nu(f*Mass, k, PS, cosmo_dic, Omega_0)
...
def func_conc_param(M, k, PS, cosmo_dic, Omega_0):
    """
    k is in units of h/Mpc, PS in (Mpc/h)^3 and M in solar_mass/h
    ...
    """
    B = 4.#5.196
    return  B * (1 + func_z_formation(M, k, PS, cosmo_dic, Omega_0) )/(1+cosmo_dic['z'])
...
def func_dens_profile_kspace(M, k, PS, cosmo_dic, hmcode_dic, Omega_0, eta_given = False):
    ...
    concentration = func_conc_param(M, k, PS, cosmo_dic, Omega_0) / (nu**eta)
...
def func_delta_char(M, k, PS, cosmo_dic, hmcode_dic, Omega_0, Omega_0_Sigma, eta_given = False): 
    """
...
def NFW_profile(M, r, k, PS, cosmo_dic, hmcode_dic, Omega_0, Omega_0_Sigma, eta_given = False):
    """
    ...
    concentration = func_conc_param(M, k, PS, cosmo_dic, Omega_0) / (nu**eta)
    normalisation = func_delta_char(M, k, PS, cosmo_dic, hmcode_dic, Omega_0, Omega_0, eta_given = eta_given) 
