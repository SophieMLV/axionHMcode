...
def func_dens_profile_ax(r_arr, M, cosmo_dic, hmcode, power_spec_dic_sigma, rho_central_param, eta_given=False):
    """
        ...
        if rho_central_param == 0:
            ...
            NFW = cosmo_dic['omega_ax_0']/cosmo_dic['omega_db_0'] * \
                          NFW_profile(M, r_arr, power_spec_dic_sigma['k'], power_spec_dic_sigma['cold'], cosmo_dic,
                                      hmcode, cosmo_dic['Omega_db_0'], cosmo_dic['Omega_db_0'], eta_given = eta_given)
...
def func_ax_halo_mass(M, cosmo_dic, hmcode, power_spec_dic_sigma, rho_central_param, eta_given=False):
    """
    M in solar_mass/h
    ...
    if isinstance(M, (int, float)) == True:
        ...
        integrand = func_dens_profile_ax(r_arr, M, cosmo_dic, hmcode, power_spec_dic_sigma, rho_central_param, eta_given=eta_given) * r_arr**2
        ...
        return [4 * np.pi * integrate.simps(y=func_dens_profile_ax(np.geomspace(1e-15, func_r_vir(M[i], cosmo_dic, cosmo_dic['Omega_db_0']), num=1000),
                                                                              M[i], cosmo_dic, hmcode, power_spec_dic_sigma, rho_central_param[i], eta_given=eta_given) * \
...
def func_central_density_param(M, cosmo_dic, power_spec_dic_sigma, eta_given=False, axion_dic = None):
    """
    M in solar_mass/h
    ...
    if isinstance(M, (int, float)) == True:
        ...
        #need a guess to find the correct central_dens_param:
        #guess is set via MaxofMc(M) = int_0_rvir \rho *r^2 dr
        #so we need the soliton and NFW part
        def integrand_ax(x):
            return func_dens_profile_ax(x, M, cosmo_dic, hmcode, power_spec_dic_sigma, 1., eta_given=eta_given)*x**2
        ...
        def func_find_root(dens):
            return func_ax_halo_mass(M, cosmo_dic, hmcode, power_spec_dic_sigma, dens, eta_given=eta_given) - MaxofMc(M, axion_dic['beta1'], axion_dic['beta2'], cosmo_dic['z'], cosmo_dic['omega_m_0'], c_frac, cosmo_dic['h'], cosmo_dic['m_ax'])
        ...
        for idx, m in enumerate(M):
            ...
            integrand_cold = NFW_profile(m, r_arr, power_spec_dic_sigma['k'], power_spec_dic_sigma['cold'], cosmo_dic, hmcode, cosmo_dic['Omega_db_0'],
                                         cosmo_dic['Omega_db_0'], eta_given = eta_given)*r_arr**2
            integral_NFW = integrate.simps(y=integrand_cold, x = r_arr)
            integral_NFW = MaxofMc(integral_NFW, axion_dic['beta1'], axion_dic['beta2'], cosmo_dic['z'], cosmo_dic['omega_m_0'], c_frac, cosmo_dic['h'], cosmo_dic['m_ax'])
...
def func_dens_profile_ax_kspace(k, M, cosmo_dic, power_spec_dic_sigma, central_dens_param, eta_given=False):
    """
    ...
    hmcode = HMCode_param_dic(cosmo_dic, power_spec_dic_sigma['k'], power_spec_dic_sigma['cold'])
    M_ax = func_ax_halo_mass(M, cosmo_dic, hmcode, power_spec_dic_sigma, central_dens_param)
    r_vir = func_r_vir(M, cosmo_dic, cosmo_dic['Omega_db_0'])

    #distinguish whether M is an array or a scalar
    if isinstance(M, (int, float)) == True:
        r_arr = np.geomspace(1e-15, r_vir, num=1000)
        dens_profile_arr = func_dens_profile_ax(r_arr, M, cosmo_dic, hmcode, power_spec_dic_sigma, central_dens_param, eta_given=eta_given) \
        ...
        for idx, m in enumerate(M):
            if  M_ax[idx] == 0:
                ...
                dens_profile_arr = func_dens_profile_ax(r_arr, m, cosmo_dic, hmcode, power_spec_dic_sigma, central_dens_param[idx], eta_given=eta_given) \
                                   * r_arr**2 * np.sin(np.outer(k, r_arr)) / np.outer(k, r_arr)
