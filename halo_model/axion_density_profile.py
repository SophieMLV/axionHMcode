...
def func_ax_halo_mass(M, cosmo_dic, power_spec_dic_sigma, M_cut, rho_central_param, hmcode_params, eta_given=False, axion_dic=None):
    ...
    if isinstance(M, (int, float)) == True:
        ...
        r_arr = np.geomspace(1e-15, r_vir, num=100000)
        ...
        for i in range(len(M)):
            ...
            R_int = np.geomspace(1e-15, upper_bound, num=100000)
...
def func_central_density_param(M, cosmo_dic, power_spec_dic_sigma, M_cut, eta_given=False, axion_dic=None):
    ...
    if isinstance(M, (int, float)) == True:
        if M < M_cut:
            ...
            xsamples = np.linspace(1e-6, r_c, 100000)
            integral_soliton = integrate.trapezoid(integrand_ax(xsamples), x=xsamples)
            
            r_arr = np.geomspace(1e-10 , 2*r_c, 100000)
        ...
        for idx, m in enumerate(M):
            if m < M_cut:
                ...
                xsamples = np.linspace(1e-6, r_c[idx], 100000)
                integral_soliton = integrate.trapezoid(integrand_ax(xsamples), x=xsamples)

                r_arr = np.geomspace(1e-15 , r_c[idx], 100000)
                integrand_cold = NFW_profile(m, r_arr, power_spec_dic_sigma['k'], power_spec_dic_sigma['cold'], cosmo_dic, hmcode_params, cosmo_dic['Omega_db_0'], 
    ...
    if isinstance(M, (int, float)) == True:
        ...
        for idx, m in enumerate(M):
            if  M_ax[idx] == 0:
                ...
                r_arr = np.geomspace(1e-15, r_vir[idx], num=100000)
